...
class PerceptronWeight(object):
    ...
    def fit(self, src_data, tgt_data, learning_rate, nepochs, lr_decay=0.0, minff=5, maxff=float("+inf")):
        ...
        ## weights used to weight samples from the two datasets
        ## weights down src-instances
        dataset_weights = {"src":len(tgt_train)/len(src_train), "tgt":1} 
        ...
        for e in range(nepochs):
            ...
            for i,example in enumerate(train, start=1):
                ...
                for f in example.features:
                    ...
                    try: # try is faster than if check for keys in dict
                        self.weights[gold_tag][f] += learning_rate*dataset_weights[type_] ## weight by domain
                    except KeyError:
                        pass
                    # and decrease weights at feature positions for prediction
                    try:
                        self.weights[pred_tag][f] -= learning_rate*dataset_weights[type_] ## weight by domain
                    except KeyError:
                        pass
            ...
            # Evaluate
            macro, micro, _ = self._evaluate(src_train+tgt_train)
